...
// google sheet id
    const ssidlist = [
    '1Vni0y4no63aMqBdbH2EUawD20TUbGRasISnO5gczVB8',  //將本行更新為你們公會的表單ID 記得放在引號裡面
]

const chlist = {
    '717409335902208051': ssidlist[0], // mom server bot channel
    '717049090441085061': ssidlist[0], // test server
    '727527306935140442': ssidlist[0], // mom server 1st boss
    '727527328598458388': ssidlist[0], // mom server 2nd boss
    '727527369148989480': ssidlist[0], // mom server 3rd boss
    '727527388338192554': ssidlist[0], // mom server 4th boss
    '727527409355718666': ssidlist[0], // mom server 5th boss
    '727529156295786547': ssidlist[0], // mom server extend knife
    '727529178861404240': ssidlist[0], // mom server fill channel
    ...
    if (message.content.substring(0, 1) === "!" || message.content.substring(0, 1) === "！") {
            ...
            if (command === 'fill' || command === '填表' || command === '傷害') {
                ...
                queue.push(async () => {
                    ...
                    //例外狀況
                        ...
                        console.log(err);
                        message.reply('請以 <!fillfor @成員 傷害數值 目標 (尾/殘/跨)> 的形式呼叫');
                    }
                })
                return;
            }
        ...
        if (command === 'help' || command === '說明') {
            var embed = {
                ...
                "image": {
                    "url": "https://cdn.discordapp.com/attachments/717049090441085061/717390849885077534/uRV4LIb5.png" //takagi
...
async function fillandreply(message, memberid, demage, object, ps) {
    try {
        ...
        if(ps === '跨'){
            table = await gapi.getDemageTableYesterday(chlist[message.channel.id]);
            former_status = await getstatus(table, memberName);
        }
        else {
            table = await gapi.getDemageTable(chlist[message.channel.id]);
            former_status = await getstatus(table, memberName);
        }
        ...
        if(ps === '跨'){
            table = await gapi.getDemageTableYesterday(chlist[message.channel.id]);
            latter_status = await getstatus(table, memberName);
        }
        else {
            table = await gapi.getDemageTable(chlist[message.channel.id]);
            latter_status = await getstatus(table, memberName);
